...
const { STRIPE_SECRET_KEY, CLIENT_URL } = require("../constants");
// Initialize Stripe only when a secret key is provided. When not provided,
// keep `stripe` null and return a clear error from checkout endpoints.
const stripe = STRIPE_SECRET_KEY ? require("stripe")(STRIPE_SECRET_KEY) : null;

module.exports.createCheckoutSession = async (req, res, next) => {
  try {
    if (!stripe) {
      // Stripe is not configured on this environment — return an informative error.
      return next(createError(500, "Stripe is not configured on the server"));
    }
    const session = await stripe.checkout.sessions.create({
      ...
      mode: "payment",
      success_url: `${CLIENT_URL}/success/${req.body.id}`,
      cancel_url: `${CLIENT_URL}/cancel/${req.body.id}`,
    });
...
module.exports.getAllOrders = async (req, res, next) => {
  try {
    ...
    const totalOrders = await Order.countDocuments(req.filter);
    ...
    res.status(200).send({ data: orders, totalOrders });
  } catch (error) {
    next(error);
  }
};
